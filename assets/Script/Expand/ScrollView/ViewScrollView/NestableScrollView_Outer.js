...
cc.Class({
    ...
    properties: {
        ...
        m_InnerPageViews: [require("NewPageView_Inner")],             //挂内嵌的PageView
    ...
    onLoad: function () {
        ...
        this.m_InnerPageViews.forEach(inner => {
            inner.setOuterScrollView(this);
        });
    },
    ...
    _findScrollingInnerSv(target) {
        ...
        for (let i = 0; i < this.m_InnerPageViews.length; i++) {
            let isHisChild = this._isHisChild(target, this.m_InnerPageViews[i].node);
            if (isHisChild) {
                return this.m_InnerPageViews[i];
            }
        }
