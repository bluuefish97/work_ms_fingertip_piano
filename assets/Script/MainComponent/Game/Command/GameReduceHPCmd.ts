...
export default class GameReduceHPCmd extends SimpleCommand {

    public execute(notification: INotification): void {
        console.log("execute:  " + "GameReduceHPCmd");

        let info = notification.getBody();                  // 解析数据并获得对应的信息
        let infoType = info.type;
        this.sendNotification(AllCommandDefine.GameReduceHPResponse, ({
            type: infoType
        }))
    };
