...
import NormalKeyUnit from "../KeyUnit/NormalKeyUnit";
import BaseKeyBaseUnit from "./BaseKeyBaseUnit";

const { ccclass, property } = cc._decorator;

@ccclass
export default class NormalKeyBaseUnit extends BaseKeyBaseUnit {
    ...
    public setPressedList(curList) {
        ...
        for (let i = 0; i < curList.length; i++) {
            ...
            // 数据初始化
            const curJS = curKey.getComponent(NormalKeyUnit);
            curJS.resetKey();
            curJS.setKeyNum(this.curKeyNum);
            curJS.setKeyType();
            curJS.setKeySpeed(this.curSpeedNum);
            // 设置高度
            curJS.setLenTime(this.lenTime);
