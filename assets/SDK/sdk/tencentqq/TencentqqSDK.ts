...
import TencentqqLoginGetToken from "./dto/TencentqqDTO";

export default class TencentqqSDK implements ServerInterface {
    ...
    startLogin() {
        ...
        qq.login({
            success: (res) => {
                console.log("XminigameSDK", "QQ 登录成功：" + JSON.stringify(res));
                let loginGetToken = new TencentqqLoginGetToken(res.code);
                this.loginSucc(loginGetToken);
            },
            fail: (res) => {
                console.log("XminigameSDK", "QQ 登录失败,", JSON.stringify(res));
                console.log("XminigameSDK", "QQ 启用游客登录");
                this.loginFail();
            }
        })
    }
